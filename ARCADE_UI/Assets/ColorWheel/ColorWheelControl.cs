...
public class ColorWheelControl : MonoBehaviour {
	...
	//The Components of the wheels
	private Material mat;
	private RectTransform RectTrans, SelectorOut;
	...
	void Start(){
		...
		SelectorOut.sizeDelta =  RectTrans.sizeDelta / 5.0f;
	...
	void Update(){
        ...
        if (dragOuter)
        {
            ...
            float rotateAngle = Mathf.Atan2(-dir.x, -dir.y);
            transform.eulerAngles = new Vector3(0, 0, -rotateAngle * 180/Mathf.PI);

            //Set the selectors positions, make sure the selectors is not rotating with color wheel
            outer = -Mathf.Atan2(-dir.x, -dir.y)+Mathf.PI/2;
	...
	public void OnClick(){
		...
		if(Vector2.Distance(RectTrans.position, Input.mousePosition) <= halfSize &&
		   Vector2.Distance(RectTrans.position, Input.mousePosition) >= halfSize - halfSize / 1.8f){
			dragOuter = true;
			return;
		}
	}
